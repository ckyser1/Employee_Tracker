...
const app = require('..functionality/functionality');
...
const removeEmployee = () => {
    ...
    conn.query('SELECT id, CONCAT(first_name," ",last_name) AS name FROM employee', async (err, res) => {
        if (err) throw err;
...
const removeRole = async () => {
    //Get roles from DB
    conn.query('SELECT title FROM role ORDER BY title asc', async (err, res) => {
        if (err) throw err;
...
const removeDept = () => {
    ...
    conn.query('SELECT name FROM department ORDER BY name asc;', async (err, res) => {
        if (err) throw err;
        //Prompt what dept to be removed
