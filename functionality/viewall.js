...
const allByMGR = () => {
    conn.query('SELECT CONCAT(employee2.first_name, " ", employee2.last_name) AS manager, employee.manager_id FROM employee INNER JOIN role on employee.role_id = role.id INNER JOIN department ON role.department_id = department.id LEFT JOIN employee AS employee2 ON employee.manager_id = employee2.id  ORDER BY department.name ASC;', async (err, res) => {
        ...
        conn.query('SELECT CONCAT(employee.first_name," " ,employee.last_name) AS full_name, manager_id FROM employee WHERE ? ORDER BY full_name ASC;', 
        {
            manager_id: mgrID
        },
        (err, res) => {
...
//View all roles
const allRole = () => {
    conn.query('SELECT title, salary, name FROM role INNER JOIN department ON role.department_id = department.id ORDER BY title ASC;', (err, res) => {
        if (err) throw err;
        //Display data
        console.log('\n');
...
const budget = () =>{
    conn.query('SELECT name FROM department', async (err, res) => {
        if(err) throw err;
        ...
        conn.query('SELECT employee.role_id, salary, department_id, department.name FROM role INNER JOIN employee on employee.role_id = role.id right JOIN department ON role.department_id = department.id WHERE ?;', 
        {name: data.budget}, 
        (err, res) => {
            if (err) throw err;
